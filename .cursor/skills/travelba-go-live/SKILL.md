...
| `TWILIO_CONTENT_SEJOUR` et la série Concierge | SIDs Utility séjour / pièces / passeport / formalité. Hors git. Sans le SID du modèle concerné, ce message ne part pas. `npx tsx scripts/arm-whatsapp-concierge.ts`, puis approbation Meta. Cron `GET /api/cron/concierge` toutes les 15 min |
