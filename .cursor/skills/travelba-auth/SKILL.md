...
WhatsApp Le Concierge : même lien, en plus de l’e-mail. Modèle Utility `TWILIO_CONTENT_CONNEXION` (`connexion_espace`) : {{1}} prénom, bouton « Ouvrir mon espace » = `https://travelba.fr/e/{{2}}`. {{2}} vaut `c/` + le code, donc l’adresse prévisualisée est `https://travelba.fr/e/c/CODE` (autre carte que `/e/CODE`). Cette page porte le titre, le descriptif et l’image. Le GET ne redirige pas : WhatsApp gardait le 307, donc seulement le domaine. L’ouverture de l’espace est le bouton de la page, en POST. Sans ce SID, l’e-mail part quand même. Ne pas réutiliser `TWILIO_WHATSAPP_CONTENT_SID`. Création : `npx tsx scripts/arm-whatsapp-connexion.ts` — le SID reste hors git. Meta doit approuver avant le premier envoi.
