...
WhatsApp Le Concierge : même lien, en plus de l’e-mail. Modèle Utility `TWILIO_CONTENT_CONNEXION` (`connexion_espace`) : {{1}} prénom, bouton « Ouvrir mon espace » = `https://travelba.fr/e/{{2}}`. {{2}} vaut `c/` + le code, donc l’adresse prévisualisée est `https://travelba.fr/e/c/CODE` (autre carte que `/e/CODE`). L’aperçu (titre, description, favicon, photo du séjour s’il y en a une) est servi au robot seulement (`WhatsApp/2.x.x.x A|I|N`, facebookexternalhit). Un tap — en-têtes de navigation, ou tout autre navigateur — redirige tout de suite vers le callback magique puis `next` (souvent la réservation). Pas de page « Ouvrir mon espace » pour ce visiteur. Le POST de la page d’aperçu ouvre aussi, si un robot a reçu le HTML. Sans ce SID, l’e-mail part quand même. Ne pas réutiliser `TWILIO_WHATSAPP_CONTENT_SID`. Création : `npx tsx scripts/arm-whatsapp-connexion.ts` — le SID reste hors git. Meta doit approuver avant le premier envoi. Les messages de séjour, pièces et formalité sont une autre série (`scripts/arm-whatsapp-concierge.ts`) : sans leur SID, rien n’est envoyé. Seul `sejour_publie` porte la photo du séjour.
