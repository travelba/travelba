...
async function ensureContent(draft: ReturnType<typeof conciergeExemplars>[number]) {
  const fromEnv = conciergeContentSid(draft.template);
...
function phraseOf(
  ...
  const card = draft.create.types["whatsapp/card"] as { body?: string; actions?: { title?: string }[] };
  let body = card.body || "";
  for (const [key, value] of Object.entries(variables)) {
    if (/^https?:|^c\//i.test(value)) continue;
    body = body.replaceAll(`{{${key}}}`, value);
  }
  return { phrase: body.replace(/\s+\n/g, "\n").trim(), button: card.actions?.[0]?.title || "" };
}
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as { only?: unknown };
    if (Array.isArray(body.only)) {
      only = body.only.filter((item): item is string => typeof item === "string");
    }
  } catch {
    only = null;
  }
  ...
  for (const draft of conciergeExemplars()) {
    if (only && !only.includes(draft.friendlyName)) continue;
    const dedupe = `catalogue:${draft.friendlyName}`;
    ...
    if (prior?.status === "sent") {
      const variables = variablesFor(draft.template, { suffix, place, reference: booking.reference, mediaUrl, firstName });
      const copy = variables ? phraseOf(draft, variables) : { phrase: "", button: "" };
      sent.push({
        nom: draft.friendlyName,
        env: draft.env,
        sid,
